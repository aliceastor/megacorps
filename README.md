...
- Decomposed sub-tasks are delegated to direct reports when the parent task is assigned to a member with subordinates. Agents can also trigger delegation by returning a strict `DELEGATE:` bullet list; MegaCorps creates child cards for direct reports and sets the parent as in progress while children run.
