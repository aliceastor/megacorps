...
- `Trash`: archived tasks, projects, agents, and machine runners, restorable in one click.
...
The `Projects` page owns project CRUD, repo settings, branch policy, runtime services, publish target, and project goals. Every project is a git repo: `gitea-local` projects are auto-provisioned on the bundled Gitea (org, repo, agent accounts, push webhook). Deleting a project archives it - it disappears everywhere, its cards stop dispatching, and it can be restored from the `Trash` page; an admin-only purge remains for truly empty projects. Agent workspaces follow the company NFS convention (`{mount}/{company}/agents/{agent}/project/{project}/`), with the company `shared/` directory for reference files; durable deliverables are committed to the project repo, not to a separate file store.
...
- In-app IP rate limiting is enabled by default. Tune `RATE_LIMIT_*` env vars or set `RATE_LIMIT_ENABLED=false` for local stress tests. With `TRUST_PROXY=true` the limiter keys on the right-most `X-Forwarded-For` hop so clients cannot spoof their bucket.
- Session cookies are `SameSite=strict`, and state-changing requests with a session cookie pass an Origin check: the Origin must match `WEB_ORIGIN`, `CSRF_TRUSTED_ORIGINS`, the request host, or the host the Next proxy forwards in `x-megacorps-web-host`. Non-browser clients without an Origin header are unaffected. Disable with `CSRF_ORIGIN_CHECK=false` if a proxy setup needs it.
- Active task runs are unique per card and kind (partial unique index), dispatch extends its execution lock while the adapter call is still running (`EXECUTION_LOCK_TTL_MS`, default 10 minutes), and runner completion retries return `duplicate: true` instead of 409.
- Agents support `maxConcurrent` (default 1) for parallel task slots, and cards support `timeoutSeconds` (30-14400) to override the default 300s adapter dispatch timeout. Webhook/OpenClaw adapter dispatches share the same timeout and no longer send secret-like adapter config values to the target URL.
- A `log-retention` cron prunes telemetry tables on an interval (`RETENTION_API_EVENTS_DAYS`, `RETENTION_PROMPT_LOGS_DAYS`, `RETENTION_CRON_RUNS_DAYS` default 90 days; `RETENTION_ACTIVITY_LOG_DAYS`/`RETENTION_TASK_LOGS_DAYS` default 0 = keep forever) and expires stale pending invites. Run it manually with `POST /api/cron/run` and `job=log-retention`.
- `GET /health` verifies database connectivity (503 when the DB is down); Docker images and compose services define healthchecks. `GET /api/cards/:id/assignment-history` lists assignee changes from the activity log.
- `scripts/backup-db.ps1` / `scripts/backup-db.sh` dump the compose PostgreSQL volume with `pg_dump -Fc` and prune old dumps.
- Database migrations are versioned: `schema_migrations` records applied versions under a Postgres advisory lock, the legacy idempotent bootstrap is version 1, and new DDL ships as new numbered versions. Existing deployments adopt the version table automatically on the next start.
- Dispatch completion (success, delegation, and failure paths) commits the agent release, card stage move, and heartbeat-run completion in one database transaction.
- Cards support scheduling: `scheduleAt` delays auto-dispatch until the given time, and `recurEveryMinutes` (5-43200) turns the card into a recurring template — the dispatch loop clones it into a normal `todo` card on each occurrence (`scheduledFromCardId` links clones to the template) and templates themselves never execute. Set both from the Kanban create modal.
- In-app notifications: card blocked, help review requested, approval pending, and budget warnings/hard stops write to a company-scoped `notifications` feed with per-user read state (`notification_reads`). The web shell shows a bell with an unread badge; `GET /api/notifications`, `POST /api/notifications/:id/read`, and `POST /api/notifications/read-all` are the APIs, and `notification.created` is pushed over the live WebSocket.
- Global search: `GET /api/search?q=...` searches cards, agents, projects, companies, chat sessions, and knowledge docs within visible companies. The web UI opens a command palette with `Ctrl/Cmd+K`.
- Dashboard charts: `GET /api/dashboard/timeseries?days=30` returns daily cost, completed cards, and run counts; the Dashboard renders cost-trend and throughput charts from it.
- Direct Chat streams partial agent output: the Hermes SSH adapter forwards stdout chunks, the server publishes throttled `chat.reply.partial` live events targeted at the requesting user only, and the chat UI renders the in-flight text inside the typing indicator.
- The web app has a global toast provider (`useToast`), and chat/org/companies/departments pages share React Query caches with live-event invalidation. UI strings are localized (zh-TW/en/ja) through `apps/web/src/lib/i18n.ts` (~460 keys per locale).
