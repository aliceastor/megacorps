...
export async function fixture(page: Page, width = 1158) {
  ...
  const positions = [0, 10, 20, 30, 50, 60, 100].map(rank => ({ id: `rank-${rank}`, companyId, name: `Position ${rank}`, slug: `rank-${rank}`, rank: ({0: 0, 10: 2, 20: 3, 30: 4, 50: 5, 60: 6, 100: 1} as Record<number, number>)[rank], isCompanyBoss: rank === 0, isCompanyLeadership: rank === 0, isActive: true, managerPositionId: ({10: 'rank-100', 20: 'rank-60', 60: 'rank-20', 50: 'rank-0', 100: 'rank-0'} as Record<number, string>)[rank] ?? null, isDepartmentHead: rank === 100, defaultDepartmentId: rank === 0 ? null : rank === 50 ? 'product' : 'engineering' }));
  const departments = [
