...
import { BriefcaseBusiness, GitBranch, Plus, Save, Target } from 'lucide-react';
...
export function ProjectsPage() {
  ...
  async function addGoal() {
    if (!companyId || !selectedProject || !goalTitle.trim()) return;
    setError('');
    try {
      await api<Goal>('/api/goals', { method: 'POST', body: JSON.stringify({ companyId, projectId: selectedProject.id, title: goalTitle.trim(), body: goalBody }) });
      setGoalTitle('');
      setGoalBody('');
      await refresh(companyId);
    } catch (err) {
  ...
  return <div style={{ display: 'grid', gap: 16 }}>
    <div className="page-head"><div><h1>Projects</h1><p>Project CRUD, repository rules, branch policy, and project goals. Workspace file paths live on the Workspace page.</p></div></div>
    ...
    <div className="data-grid">
      ...
      <section className="card section-card">
        <h2><Target size={18} /> Project Goals</h2>
        <div className="meta-grid">
          <span>Selected project <b>{selectedProject?.name ?? 'No project'}</b></span>
          <span>Project goals <b>{selectedProjectGoals.length}</b></span>
        </div>
        ...
        <button className="btn btn-primary" disabled={!selectedProject || !goalTitle.trim()} onClick={addGoal}><Plus size={15} /> Add Project Goal</button>
        <div className="table-list">
          {selectedProjectGoals.map((goal) => <div className="list-row" key={goal.id}>
            <b>{goal.title}</b>
