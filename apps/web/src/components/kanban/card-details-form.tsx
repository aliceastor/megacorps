...
export function CardDetailsForm({
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    ...
    <div className="form-grid">
      <label className="field-label">{t('kanban.assignee')}<select className="input" data-field="assigneeId" value={draft?.assigneeId ?? ''} onChange={(e) => setDraft({ ...(draft ?? {}), assigneeId: e.target.value || null, reviewerId: draft?.reviewerId === e.target.value ? null : draft?.reviewerId, reviewerIds: (draft?.reviewerIds ?? []).filter((id) => id !== e.target.value) })}><option value="">{t('kanban.assignee')}</option>{agents.filter(agent => agent.companyId === selected.companyId).map((agent) => <option value={agent.id} key={agent.id}>{agent.name}</option>)}</select></label>
      <label className="field-label">{t('kanban.reviewer')}<select className="input" data-field="reviewerId" value={draft?.reviewerId ?? ''} onChange={(e) => setDraft({ ...(draft ?? {}), reviewerId: e.target.value || null })}><option value="">{t('kanban.reviewer')}</option>{agents.filter(agent => agent.companyId === selected.companyId && agent.id !== draft?.assigneeId).map((agent) => <option value={agent.id} key={agent.id}>{agent.name}</option>)}</select></label>
    ...
    <div className="form-grid">
      ...
      <label className="check-row" style={{ alignSelf: 'end' }}><input type="checkbox" data-field="requiresApproval" checked={Boolean(draft?.requiresApproval)} onChange={(e) => setDraft({ ...(draft ?? {}), requiresApproval: e.target.checked })} /> {t('forms.clientApproval')}</label>
    </div>
    {!fieldsOnly && <>
