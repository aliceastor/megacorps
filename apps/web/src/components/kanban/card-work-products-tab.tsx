  ...
  return <div style={{ display: 'grid', gap: 10 }}>
    ...
    <section className="section-card" style={{ padding: 0 }}>
      <div className="form-grid">
        <label className="field-label">{t('kanban.type')}<select className="input" value={workProductType} onChange={(event) => setWorkProductType(event.target.value as WorkProductType)}>{workProductTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
