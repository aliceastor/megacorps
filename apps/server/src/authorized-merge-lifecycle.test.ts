...
function fixture(t: TestContext) {
  ...
  return { project, card, state, plan, fetchImpl, posts: () => posts, setPost: (fn: typeof post) => { post = fn; }, merge: () => { merged = true; }, drift: () => { observedHead = 'b'.repeat(40); }, retarget: () => { observedBase = 'other'; } };
}
...
test('retargeting in the final POST gap never claims Done or cancellation of the accepted external effect', async (t) => {
  const f = fixture(t);
