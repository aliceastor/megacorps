...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/runner/task-runs/:id/complete', group: 'Runner Agent API', auth: 'runner', summary: 'Accepts optional structured report and normalized workProducts; required children, review and merge evidence still gate Done. Complete a runner-claimed task-run and attach work products such as branches, commits, PRs, reports, screenshots, or artifacts. Dispatch success/done queues quality review when the card has a distinct reviewer; review success approves to done. status=waiting_on_external releases the run lock while CI/CD, deploy, export, or outside approval continues, with optional pollIntervalSeconds chosen by the agent/runtime.', params: { id: 'Task-run UUID.' }, body: { status: 'success | failed | cancelled | done | blocked | needs_review | in_review | waiting_on_external', summary: 'Short result', output: 'Full output/log', costUsd: 0.01, pollIntervalSeconds: 'Optional 30-86400 seconds when status=waiting_on_external.', workProducts: [{ type: 'pull_request', title: 'PR #42', pullRequestUrl: 'https://github.com/org/repo/pull/42', branch: 'megacorps/card-1234-alice' }] } },
