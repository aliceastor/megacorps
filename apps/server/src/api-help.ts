...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'DELETE', path: '/api/companies/:id', group: 'Companies', auth: 'session', requiredRole: 'admin', summary: 'Delete an empty company. Memberships, scaffold positions, activity rows, and the built-in Local Mock Runtime are removed automatically; real company-owned content blocks deletion and is returned in company_not_empty.blocking.', params: { id: 'Company UUID.' } },
  ...
  { method: 'PUT', path: '/api/cards/:id', group: 'Kanban', auth: 'session', summary: 'Update a Kanban task. Include updatedAt for optimistic locking. Goal selection is validated against the card company, department, and project. Parent tasks cannot move to done or in_review while required child cards are incomplete; the API returns 409 parent_children_incomplete. Manual recovery can move cancelled tasks to done when an operator accepts preserved output.', params: { id: 'Task UUID.' }, body: { title: 'Updated title', body: 'Updated detail', columnStatus: 'todo | in_progress | in_review | needs_review | waiting_on_external | done | blocked | cancelled', rollupStatus: 'planning | delegated | waiting_on_children | waiting_on_dependencies | waiting_on_external | integrating | ready_for_review | done | blocked | null', projectId: 'uuid | null', goalId: 'uuid | null', decisionMode: 'execute | delegate | hybrid | review | integrate | null', requiredChildPolicy: 'all_required_accepted | all_non_cancelled_accepted | threshold | manual', childRequirementLevel: 'required | optional | follow_up', estimatedWeight: 1, estimatedDurationMinutes: 30, taskBudgetLimit: 2.5, revisionCount: 1, maxRevisions: 3, updatedAt: 'ISO datetime from existing card' } },
  ...
  { method: 'POST', path: '/api/cards/:id/run', group: 'Kanban', auth: 'session', summary: 'Queue a dispatch task-run attempt for the background worker. First adapter turn gets full context; later scoped adapter sessions for codex-app/hermes/hermes-ssh get a fresh DB delta prompt.', params: { id: 'Task UUID.' } },
  { method: 'POST', path: '/api/cards/:id/review', group: 'Kanban', auth: 'session', summary: 'Queue a review task-run attempt for the background worker. First review turn gets full context; later scoped adapter sessions for codex-app/hermes/hermes-ssh get a fresh DB delta prompt.', params: { id: 'Task UUID.' } },
  ...
  { method: 'POST', path: '/api/webhook/task-complete', group: 'Kanban', auth: 'none', summary: 'External agent callback to report task progress, delegation, review, external wait, or completion. Send taskRunId for idempotent processing. A strict DELEGATE: bullet block in summary/output creates child cards for active direct reports and keeps the parent in_progress; task runtimes should not call session-auth /api/cards themselves. status=done/in_review queues quality review instead of marking done when a distinct reviewer exists. status=waiting_on_external releases locks while a PR/CI/deploy/approval waits; include pollIntervalSeconds when the agent wants polling instead of a tight global heartbeat; status=needs_review means the assignee cannot finish and needs reviewer guidance; blocked with guidance/escalation wording is also promoted to help review when a reviewer exists, or accepted as done when there is no higher reviewer/manager.', body: { cardId: 'uuid', taskRunId: 'task-run uuid from prompt', status: 'done | blocked | needs_review | in_review | in_progress | waiting_on_external | todo | cancelled', summary: 'Short result, delegation summary, or needs reviewer guidance. Include DELEGATE: here or in output to create child cards.', output: 'Full output/log with attempted methods, blocker, reviewer questions, partial output, or DELEGATE: bullet list', costUsd: 0.05, pollIntervalSeconds: 'Optional 30-86400 seconds when status=waiting_on_external.', workProducts: [{ type: 'pull_request', title: 'PR for task', pullRequestUrl: 'https://github.com/org/repo/pull/42', branch: 'megacorps/card-1234-alice', commitSha: 'abc123' }] } },
  ...
  { method: 'POST', path: '/api/chat/sessions/:id/messages', group: 'Chat', auth: 'session', summary: 'Send a message to an agent and store the response. First turn injects full company/goal/Kanban context; subsequent turns with an adapter session hand context management to Hermes/Codex and send only the latest user message.', params: { id: 'Chat session UUID.' }, body: { body: 'Message for the agent' } },
...
const currentArchitecture = {
  ...
  sourceOfTruth: [
    ...
    'Kanban: normalized task lifecycle, recursive card tree, required child policy, dependency graph, action timeline, assignee/reviewer, project/goal context, work products, message board comments, ticket-thread actions/logs, context snapshots, manual run/review/decompose, strict DELEGATE child-card creation, and scoped adapter-session delta prompts for repeat turns.',
  ...
  surfaces: [
    ...
    { name: 'Kanban', route: '/kanban', purpose: 'Task creation, recursive lifecycle, assignment, project/goal context, message board, ticket thread, work products, rollup, context snapshots/requests, external waits, required tools, integrations, run/review/decompose, and adapter-session delta prompts after the first turn.', primaryApi: ['/api/cards', '/api/cards/:id/run', '/api/cards/:id/review', '/api/cards/:id/work-products', '/api/cards/:id/rollup', '/api/cards/:id/context'] },
    ...
    { name: 'Direct Chat', route: '/chat', purpose: 'Project-scoped or no-project agent chat sessions. First turn bootstraps full context; later turns hand context management to the adapter session and send only the latest user message.', primaryApi: ['/api/chat/sessions', '/api/chat/sessions/:id/messages'] },
  ...
  multiAgentNotes: [
    ...
    'Direct Chat uses full_bootstrap on the first turn and adapter_session_continuation on later turns when a chat adapter session exists. This avoids repeatedly injecting the same history/context.',
    'Kanban dispatch/review uses full_bootstrap on the first scoped card session and adapter_session_delta on later turns for codex-app, hermes, and hermes-ssh. The delta refreshes DB truth such as stage, lastError, review feedback, child/dependency state, messages, actions, logs, and work products.',
  ...
  remainingGaps: [
    ...
    'Hermes/Hermes-SSH receive scoped session ids for Direct Chat and Kanban delta prompts, but true resume behavior depends on the Hermes runtime honoring that session id. Add adapter-level resume flags/API calls if Hermes requires them.',
...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  ...
  if (endpoint.path.includes('/integrations')) {
    const integration = { id: 'integration-uuid', parentCardId: 'card-uuid', summary: 'Integrated accepted child outputs.', status: 'accepted', conflictNotes: null };
  ...
  if (endpoint.path.includes('/webhook/task-complete')) {
    return { responseSchema: { ok: 'boolean', duplicate: 'boolean optional', cardId: 'uuid', taskRunId: 'uuid optional', requestedStatus: 'CardStatus', newStatus: 'CardStatus', reviewerId: 'uuid | null optional', delegated: 'boolean optional', delegationFailed: 'boolean optional', childCount: 'number optional' }, responseExample: { ok: true, cardId: 'card-uuid', taskRunId: 'task-run-uuid', requestedStatus: 'done', newStatus: 'in_review', reviewerId: 'reviewer-uuid', delegated: false, delegationFailed: false, childCount: 0 }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
