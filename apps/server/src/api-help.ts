...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/webhook/task-complete', group: 'Kanban', auth: 'none', summary: 'External agent callback to report task progress/completion. Send taskRunId for idempotent completion processing. status=waiting_on_external releases locks while a PR/CI/deploy/approval waits; status=needs_review means the assignee cannot finish and needs reviewer guidance; blocked with guidance/escalation wording is also promoted to help review when a reviewer exists, or accepted as done when there is no higher reviewer/manager.', body: { cardId: 'uuid', taskRunId: 'task-run uuid from prompt', status: 'done | blocked | needs_review | in_review | in_progress | waiting_on_external | todo | cancelled', summary: 'Short result or needs reviewer guidance', output: 'Full output/log with attempted methods, blocker, reviewer questions, partial output', costUsd: 0.05, workProducts: [{ type: 'pull_request', title: 'PR for task', pullRequestUrl: 'https://github.com/org/repo/pull/42', branch: 'megacorps/card-1234-alice', commitSha: 'abc123' }] } },
