...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'PUT', path: '/api/projects/:id/workspace-files', group: 'Workspace', auth: 'session', summary: 'Push or replace one project shared workspace file. This is the portable transport for Hermes/Codex agents on other hosts; local /workspaces paths are authority paths, not required shared mounts.', params: { id: 'Project UUID.' }, body: { path: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/report.md', body: '# Report\\n...', contentType: 'text/markdown', encoding: 'utf-8 | base64', cardId: 'Optional Kanban card UUID in the same project.', metadata: { source: 'agent-slug' } } },
  { method: 'DELETE', path: '/api/projects/:id/workspace-files', group: 'Workspace', auth: 'session', summary: 'Delete one project shared workspace file by authority path.', params: { id: 'Project UUID.' }, query: { path: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/report.md' } },
