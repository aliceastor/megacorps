...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/cards/:id/work-products', group: 'Kanban', auth: 'session', summary: 'Attach a reviewable work product to a task. Prefer Git/URL/project shared workspace metadata over local-only paths for multi-system agents. Project non-code deliverables should use /workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/... or another durable URL/path.', params: { id: 'Task UUID.' }, body: { type: 'pull_request | commit | preview_url | report | screenshot | artifact | file | external', title: 'PR #42', summary: 'What changed', url: 'https://...', repoUrl: 'https://github.com/org/repo', branch: 'megacorps/card-1234-alice', commitSha: 'abc123', pullRequestUrl: 'https://github.com/org/repo/pull/42' } },
  ...
  { method: 'GET', path: '/api/projects/:id/workspace-files', group: 'Workspace', auth: 'session', summary: 'List or pull project shared workspace files over HTTP. Use Authorization: Bearer MEGACORPS_API_TOKEN for cross-host agents. Send path to pull one file body; send prefix to list metadata without body by default.', params: { id: 'Project UUID.' }, query: { path: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/report.md optional', prefix: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/ optional', includeBody: 'Optional boolean. Defaults true for path pulls and false for prefix lists.', limit: '1-500, default 200.' } },
  { method: 'PUT', path: '/api/projects/:id/workspace-files', group: 'Workspace', auth: 'session', summary: 'Push or replace one project shared workspace file. This is the portable transport for Hermes/Codex agents on other hosts; local /workspaces paths are authority paths, not required shared mounts. Writes are version-gated so concurrent agents cannot silently overwrite each other: send expectedVersion 0 to create, or the version you read to replace. 409 workspace_file_version_required means you wrote to an existing file without a version; 409 workspace_file_version_conflict means it changed since you read it, so re-read, merge, and write again.', params: { id: 'Project UUID.' }, body: { path: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/report.md', body: '# Report\\n...', contentType: 'text/markdown', encoding: 'utf-8 | base64', cardId: 'Optional Kanban card UUID in the same project.', expectedVersion: '0 to create, or the version returned by GET to replace.', holder: 'Optional advisory lock holder label if you hold the lock.', metadata: { source: 'agent-slug' } } },
  { method: 'DELETE', path: '/api/projects/:id/workspace-files', group: 'Workspace', auth: 'session', summary: 'Delete one project shared workspace file by authority path. Refused with 409 workspace_file_locked while another holder has an unexpired advisory lock, and with 409 workspace_file_version_conflict when expectedVersion no longer matches.', params: { id: 'Project UUID.' }, query: { path: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/report.md', expectedVersion: 'Optional version guard.', holder: 'Optional advisory lock holder label if you hold the lock.' } },
  { method: 'POST', path: '/api/projects/:id/workspace-files/lock', group: 'Workspace', auth: 'session', summary: 'Take or renew a TTL-bounded advisory lock on one workspace file before a long edit. 409 workspace_file_locked when another holder still has it. Locks expire on their own, so they are a courtesy to other agents, not a replacement for expectedVersion.', params: { id: 'Project UUID.' }, body: { path: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/report.md', holder: 'agent-slug', ttlSeconds: 300 } },
  { method: 'DELETE', path: '/api/projects/:id/workspace-files/lock', group: 'Workspace', auth: 'session', summary: 'Release an advisory lock you hold on one workspace file.', params: { id: 'Project UUID.' }, query: { path: '/workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/report.md', holder: 'agent-slug' } },
  { method: 'DELETE', path: '/api/projects/:id', group: 'Projects', auth: 'session', summary: 'Delete a project that has no cards, work products, chat sessions, cost events, prompt logs, or workspace files. Project goals are removed with the project.', params: { id: 'Project UUID.' } },
...
const currentArchitecture = {
  ...
  sourceOfTruth: [
    ...
    'Workspace: company/project folder manager and authoritative non-coding project-file namespace. Use /workspaces/{companySlug}/{projectSlug}/deliverables/{cardId}/ for durable task deliverables; cross-host agents pull/push files through /api/projects/:id/workspace-files with Bearer API tokens because runtime-local clone/scratch paths are not shared truth.',
  ...
  surfaces: [
    ...
    { name: 'Workspace', route: '/workspaces', purpose: 'Company/project shared file authority for non-coding project files and durable deliverable paths.', primaryApi: ['/api/companies', '/api/projects', '/api/projects/:id/workspace-files', '/api/cards/:id/work-products'] },
  ...
  multiAgentNotes: [
    'Use project repoUrl and workPath as the shared coding authority; use project shared workspace paths for durable non-code deliverables; remote agents should pull/push those files through /api/projects/:id/workspace-files with Authorization: Bearer MEGACORPS_API_TOKEN. Use runtime local roots only for machine-local clone/cache/scratch placement.',
  ...
  remainingGaps: [
    'Project shared file space is addressed by authority paths under /workspaces/{companySlug}/{projectSlug}/ and transported by /api/projects/:id/workspace-files. Agents should attach durable deliverables as workProducts with project shared workspace paths or URLs; /tmp and runtime-local scratch paths are temporary only.',
...
function entityFromEndpoint(endpoint: ApiEndpoint): string {
  ...
  if (endpoint.path.includes('/workspace-files')) return 'workspaceFile';
...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  ...
  if (endpoint.path.includes('/workspace-files')) {
    const file = { id: 'workspace-file-uuid', companyId: 'company-uuid', projectId: 'project-uuid', cardId: 'card-uuid', path: '/workspaces/mega-corps/tubelike-research/deliverables/card-uuid/report.md', body: '# Report\n...', contentType: 'text/markdown', encoding: 'utf-8', sizeBytes: 12, metadata: {}, version: 3, lockedBy: null, lockExpiresAt: null, updatedAt: '2026-06-14T00:00:00.000Z' };
    if (endpoint.method === 'GET') return { responseSchema: { file: 'WorkspaceFile when path is provided', files: 'WorkspaceFile[] when listing prefix metadata' }, responseExample: { file }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
    return { responseSchema: { file }, responseExample: { file }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
