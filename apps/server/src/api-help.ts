...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'GET', path: '/api/prompt-logs', group: 'Logs', auth: 'session', summary: 'Read redacted outbound prompt snapshots sent to adapters for visible companies.', query: { companyId: 'Optional company UUID.', cardId: 'Optional task UUID.', agentId: 'Optional agent UUID.', source: 'dispatch | review | chat | test.', limit: '1-500, default 200.' } },
  ...
  { method: 'POST', path: '/api/cards/:id/external-waits', group: 'External Events', auth: 'session', summary: 'Move a card to waiting_on_external, release execution locks, and record what external system should wake it.', params: { id: 'Task UUID.' }, body: { waitingFor: 'GitHub Actions CI', provider: 'github', externalId: 'run-id optional', externalUrl: 'https://github.com/org/repo/actions/runs/123', timeoutAt: 'optional ISO datetime' } },
  ...
  { method: 'POST', path: '/api/webhook/task-complete', group: 'Kanban', auth: 'none', summary: 'External agent callback to report task progress, delegation, review, external wait, or completion. Send taskRunId for idempotent processing. A strict DELEGATE: bullet block in summary/output creates child cards for active direct reports and keeps the parent in_progress; task runtimes should not call session-auth /api/cards themselves. status=done/in_review queues quality review instead of marking done when a distinct reviewer exists. status=waiting_on_external releases locks while a PR/CI/deploy/approval waits; status=needs_review means the assignee cannot finish and needs reviewer guidance; blocked with guidance/escalation wording is also promoted to help review when a reviewer exists, or accepted as done when there is no higher reviewer/manager.', body: { cardId: 'uuid', taskRunId: 'task-run uuid from prompt', status: 'done | blocked | needs_review | in_review | in_progress | waiting_on_external | todo | cancelled', summary: 'Short result, delegation summary, or needs reviewer guidance. Include DELEGATE: here or in output to create child cards.', output: 'Full output/log with attempted methods, blocker, reviewer questions, partial output, or DELEGATE: bullet list', costUsd: 0.05, workProducts: [{ type: 'pull_request', title: 'PR for task', pullRequestUrl: 'https://github.com/org/repo/pull/42', branch: 'megacorps/card-1234-alice', commitSha: 'abc123' }] } },
  ...
  { method: 'POST', path: '/api/runner/task-runs/:id/complete', group: 'Runner Agent API', auth: 'runner', summary: 'Complete a runner-claimed task-run and attach work products such as branches, commits, PRs, reports, screenshots, or artifacts. Dispatch success/done queues quality review when the card has a distinct reviewer; review success approves to done. status=waiting_on_external releases the run lock while CI/CD, deploy, export, or outside approval continues.', params: { id: 'Task-run UUID.' }, body: { status: 'success | failed | cancelled | done | blocked | needs_review | in_review | waiting_on_external', summary: 'Short result', output: 'Full output/log', costUsd: 0.01, workProducts: [{ type: 'pull_request', title: 'PR #42', pullRequestUrl: 'https://github.com/org/repo/pull/42', branch: 'megacorps/card-1234-alice' }] } },
...
const currentArchitecture = {
  ...
  sourceOfTruth: [
    ...
    'Kanban: normalized task lifecycle, recursive card tree, required child policy, dependency graph, action timeline, assignee/reviewer, project/goal context, work products, comments, context snapshots, manual run/review/decompose, and strict DELEGATE: child-card creation for agents with direct reports.',
    'External Events: waiting_on_external cards release execution locks while CI/CD, deployment, exports, or outside approvals complete; external events wake cards into review, rework, done, or blocked.',
