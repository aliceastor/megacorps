...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/runner/task-runs/:id/complete', group: 'Runner Agent API', auth: 'runner', summary: 'Complete a runner-claimed task-run and attach work products such as branches, commits, PRs, reports, screenshots, or artifacts. status=waiting_on_external releases the run lock while CI/CD, deploy, export, or outside approval continues.', params: { id: 'Task-run UUID.' }, body: { status: 'success | failed | cancelled | done | blocked | needs_review | in_review | waiting_on_external', summary: 'Short result', output: 'Full output/log', costUsd: 0.01, workProducts: [{ type: 'pull_request', title: 'PR #42', pullRequestUrl: 'https://github.com/org/repo/pull/42', branch: 'megacorps/card-1234-alice' }] } },
...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  ...
  if (endpoint.path.includes('/webhook/task-complete')) {
    return { responseSchema: { ok: 'boolean', duplicate: 'boolean optional', cardId: 'uuid', taskRunId: 'uuid optional', requestedStatus: 'CardStatus', newStatus: 'CardStatus', reviewerId: 'uuid | null optional', delegated: 'boolean optional', delegationFailed: 'boolean optional', childCount: 'number optional' }, responseExample: { ok: true, cardId: 'card-uuid', taskRunId: 'task-run-uuid', requestedStatus: 'in_progress', newStatus: 'in_progress', reviewerId: null, delegated: true, delegationFailed: false, childCount: 2 }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
