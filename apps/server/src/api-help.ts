...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/departments', group: 'Departments', auth: 'session', summary: 'Create a department. Assign agents through PUT /api/agents/:id with departmentId and bossId. headAgentId names the department head (receives department cards from the CEO and splits them to members); description is the charter the CEO reads to decide which departments a task or brainstorm concerns.', body: { companyId: 'uuid', name: 'Engineering', slug: 'engineering', headRolePrompt: 'Optional additive head role instructions, null or at most 8000 characters.', headAgentId: 'agent uuid optional', description: 'What this department is responsible for.' } },
  { method: 'PUT', path: '/api/departments/:id', group: 'Departments', auth: 'session', summary: 'Update a department: rename, set or clear the department head, or the charter description.', params: { id: 'Department UUID.' }, body: { name: 'IT', headRolePrompt: 'Optional additive head role instructions, null or at most 8000 characters.', headAgentId: 'agent uuid | null', description: 'What this department is responsible for.' } },
