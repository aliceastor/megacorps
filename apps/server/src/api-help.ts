...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  ...
  if (endpoint.path === '/api/runner/task-runs/claim') {
    return { responseSchema: { taskRun: 'TaskRun | null (no eligible work)', card: 'Card, only on successful claim', agent: 'Redacted agent, only on successful claim', project: 'Project | null, only on successful claim', runtime: 'Redacted runtime | null, only on successful claim', companyContext: 'Bounded current prompt string, only on successful claim' }, responseExample: { taskRun: { id: 'task-run-uuid', status: 'running' }, card: { id: 'card-uuid' }, agent: { id: 'agent-uuid' }, project: null, runtime: null, companyContext: 'Current company role and selected knowledge context.' }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
