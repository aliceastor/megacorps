...
type ApiEndpoint = {
  ...
  path: string;
  group: string;
  auth: 'none' | 'session' | 'runner' | 'agent-session' | 'webhook' | 'gitea-token' | 'a2a-signature';
...
const cliHelp = {
  ...
  manifestExample: [
    ...
    '    role: worker',
    '    adapterType: a2a',
    '    runtimeId: REPLACE_WITH_EXPLICIT_COMPANY_RUNTIME_UUID',
    'cards:',
    '  - company: example',
    '    project: Web App',
    '    assignee: builder',
...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/auth/signup', group: 'Auth', auth: 'none', summary: 'Create a user when DB auth.signup_enabled=true. If no active admin exists, this account becomes global admin without any implicit company membership; the admin can create the first company.', body: { email: 'user@example.com', name: 'Operator', password: 'at least 8 chars' } },
  ...
  { method: 'GET', path: '/api/admin/activity', group: 'Admin', auth: 'session', requiredRole: 'admin', summary: 'Read global audit records, including companyless administration and preserved company deletion history. view=summary returns bounded metadata without details; ordinary company-scoped activity never exposes these rows.', query: { view: 'summary opts into {items,nextCursor}; omitted preserves the legacy array.', limit: 'Summary: 1-100, default 50. Legacy: 1-500, default 200.', cursor: 'Opaque exact createdAt+id cursor; summary mode only.', q: 'Server-side action/entity/actor/details search, max 200 characters.' } },
  { method: 'GET', path: '/api/admin/activity/:id', group: 'Admin', auth: 'session', requiredRole: 'admin', summary: 'Read one companyless global audit record. A global admin session is required.', params: { id: 'Activity UUID.' } },
  ...
  { method: 'GET', path: '/api/system-logs', group: 'Logs', auth: 'session', summary: 'Read persisted API lifecycle logs for the current user. view=summary excludes requestBody/responseBody and returns {items,nextCursor}; use the detail route to fetch one payload.', query: { view: 'summary opts into metadata pagination; omitted preserves the legacy array.', limit: 'Summary: 1-100, default 50. Legacy: 1-500, default 100.', cursor: 'Opaque exact createdAt+id cursor; summary mode only.', q: 'Server-side method/path/error search, max 200 characters.' } },
  { method: 'GET', path: '/api/system-logs/:id', group: 'Logs', auth: 'session', summary: 'Read one API lifecycle log, including bounded stored request/response bodies, for the same user that owns it.', params: { id: 'API event UUID.' } },
  { method: 'POST', path: '/api/agents/:id/prompt-preview', group: 'Agents', auth: 'session', summary: 'Compute a complete redacted fresh-invocation prompt from saved agent/company/project settings using the real task or Direct Chat builders and adapter envelope. Company viewer membership required; projects must belong to the same company. Synthetic invocation IDs are not persisted. Hermes/runtime-owned system context is outside the preview. Creates no cards, chats, runs, prompt logs or tokens and makes no adapter/model calls.', body: { kind: 'task | chat', projectId: 'Optional same-company project UUID or null.', title: 'Optional task title, max 1000 characters.', body: 'Required task body or chat message, 1–100000 characters.' } },
  { method: 'GET', path: '/api/prompt-logs', group: 'Logs', auth: 'session', summary: 'Read redacted outbound prompt snapshots for visible companies. view=summary excludes prompt/metadata at SELECT time and returns bounded title/preview/contextMode metadata in {items,nextCursor}; use the detail route for one full redacted prompt.', query: { view: 'summary opts into metadata pagination; omitted preserves the legacy array.', companyId: 'Optional visible company UUID.', cardId: 'Optional task UUID.', agentId: 'Optional agent UUID.', source: 'dispatch | review | chat | test.', surface: 'kanban | chat.', q: 'Server-side title/source/adapter/prompt and identity search, max 200 characters.', limit: 'Summary: 1-100, default 50. Legacy: 1-500, default 200.', cursor: 'Opaque exact createdAt+id cursor; summary mode only.' } },
  { method: 'GET', path: '/api/prompt-logs/:id', group: 'Logs', auth: 'session', summary: 'Read one full redacted prompt log only when its company is visible to the current user.', params: { id: 'Prompt log UUID.' } },
  { method: 'GET', path: '/api/activity', group: 'Logs', auth: 'session', summary: 'Read visible-company product activity. view=summary excludes details and returns {items,nextCursor}; company-null global audit remains available only through /api/admin/activity.', query: { view: 'summary opts into metadata pagination; omitted preserves the legacy array.', companyId: 'Optional visible company UUID.', entityType: 'Optional entity type.', q: 'Server-side action/entity/actor/details search, max 200 characters.', limit: 'Summary: 1-100, default 50. Legacy: 1-500, default 200.', cursor: 'Opaque exact createdAt+id cursor; summary mode only.' } },
  { method: 'GET', path: '/api/activity/:id', group: 'Logs', auth: 'session', summary: 'Read one company-scoped activity detail only when that company is visible. This route never returns company-null global audit.', params: { id: 'Activity UUID.' } },
  ...
  { method: 'GET', path: '/api/heartbeat-runs', group: 'Logs', auth: 'session', summary: 'Read visible-company heartbeat history. view=summary returns bounded metadata in {items,nextCursor}.', query: { view: 'summary opts into metadata pagination; omitted preserves the legacy array.', companyId: 'Optional visible company UUID.', cardId: 'Optional task UUID.', agentId: 'Optional agent UUID.', status: 'Optional run status.', q: 'Server-side source/status/error/identity search, max 200 characters.', limit: 'Summary: 1-100, default 50. Legacy: 1-500, default 200.', cursor: 'Opaque exact createdAt+id cursor; summary mode only.' } },
  { method: 'GET', path: '/api/heartbeat-runs/:id', group: 'Logs', auth: 'session', summary: 'Read one heartbeat run only when its company is visible.', params: { id: 'Heartbeat run UUID.' } },
  { method: 'GET', path: '/api/task-runs', group: 'Logs', auth: 'session', summary: 'Read visible-company task-run attempts. view=summary excludes output at SELECT time and returns a bounded output preview in {items,nextCursor}. Legacy array clients remain compatible.', query: { view: 'summary opts into metadata pagination; omitted preserves the legacy array.', companyId: 'Optional visible company UUID.', cardId: 'Optional task UUID.', agentId: 'Optional agent UUID.', kind: 'dispatch | review | message | message_review | panel_review.', status: 'queued | running | success | failed | cancelled.', q: 'Server-side kind/source/status/error/output and identity search, max 200 characters.', limit: 'Summary: 1-100, default 50. Legacy: 1-500, default 200.', cursor: 'Opaque exact createdAt+id cursor; summary mode only.' } },
  { method: 'GET', path: '/api/task-runs/:id', group: 'Logs', auth: 'session', summary: 'Read one task run, including output, only when its company is visible.', params: { id: 'Task run UUID.' } },
  { method: 'GET', path: '/api/cost-events', group: 'Budget', auth: 'session', summary: 'Read visible-company attempt ledger rows, newest occurredAt then id. costUsd is an exact eight-decimal string or null (unknown); costStatus and usage.costSource/tokenSource distinguish runtime-reported actual, estimates and unknown. Actual is not invoice reconciliation. Original attemptKey, taskRunId, heartbeatRunId, runtimeId and reportingSource are immutable; A2A context is not an attempt. Pre-migration rows have source legacy_fixed_rate and no invented attempt identity.', query: { companyId: 'Optional company UUID.', agentId: 'Optional agent UUID.', cardId: 'Optional direct card UUID; excludes child cards.', projectId: 'Optional project UUID; includes each child attempt once.', limit: '1-500, default 200.', offset: '0-1000000, default 0. Offset pagination may shift if settlements arrive during a capture.' } },
  { method: 'GET', path: '/api/usage-summary', group: 'Budget', auth: 'session', summary: 'Exact ledger aggregates: actualUsd, estimatedUsd, totalUsd (known subtotal), reservedUsd, unknownAttempts and attempts. Runtime-reported actual is not invoice reconciliation. period contains UTC start inclusive/end exclusive or null for all time. reservedUsd is current outstanding exposure at reservationAsOf across periods. Unknown costs and expired reservations cannot guarantee a provider billing ceiling. Card limits apply to cumulative direct executions; company and project totals include child attempts once. Warning-only policies never stop admission; disabled policies do not apply; manual pause is independent of UTC budget recovery.', query: { companyId: 'Optional visible company UUID; foreign company is forbidden.', agentId: 'Optional original agent UUID.', cardId: 'Optional direct card UUID.', projectId: 'Optional original project UUID.', period: 'YYYY-MM or all. Default current UTC month.' } },

  { method: 'POST', path: '/api/company-setup', group: 'Companies', auth: 'session', requiredRole: 'operator', summary: 'Start or resume an idempotent company setup draft. No implicit company exists after bootstrap. setupKey is a client-generated UUID scoped by authorized company membership on retries.', body: {setupKey:'UUID',name:'Studio',slug:'studio',mission:'Build useful things'} },
  { method: 'GET', path: '/api/companies/:id/setup', group: 'Companies', auth: 'session', requiredRole: 'operator', summary: 'Read persisted entities and current setup status: draft, ready, needs_attention, or dispatch_disabled. Read-only; never pauses dispatch or changes completion history. Registered runtime metadata is not proof of a live connection.', params: {id:'Company UUID'} },
  { method: 'PUT', path: '/api/companies/:id/setup', group: 'Companies', auth: 'session', requiredRole: 'operator', summary: 'Save sequential setup without duplicate entities. Reopen explicitly pauses dispatch, retains entity IDs and requires fresh checks. Runtime selection never edits a shared runtime; new A2A runtimeCreateKey UUID identifies an immutable creation retry (omit only for legacy payload-derived idempotency). Finish requires distinct Boss/head, structure and recent successful connection checks. Repository write and protected-merge readiness remain separate.', params: {id:'Company UUID'}, body:{step:'company | boss | department | head | runtime | finish | reopen',runtimeCreateKey:'UUID for a new runtime operation; reuse unchanged on retry'} },
  { method: 'POST', path: '/api/companies/:id/setup/probe', group: 'Companies', auth: 'session', requiredRole: 'operator', summary: 'Explicit bounded GET of both A2A agent-card endpoints. No model task is executed. Success proves endpoint response only; other adapters use the existing explicit, potentially billable agent test-connection endpoint.', params: {id:'Company UUID'} },
  ...
  { method: 'GET', path: '/api/companies/:id/execution-readiness', group: 'Companies', auth: 'session', summary: 'Read structural/runtime readiness and separate worker repository write capability. Draft headless departments remain setup issues unless selected. Read-only provider inspection never writes policies or reveals credentials.', params: { id: 'Visible company UUID.' }, query: { agentId: 'Optional worker UUID.', departmentId: 'Optional selected department UUID.', projectId: 'Optional project UUID for repository capability inspection.' } },
  { method: 'POST', path: '/api/companies', group: 'Companies', auth: 'session', summary: 'Create a company with automatic dispatch disabled until minimum setup is ready. panelReviewDefault decides when a single-mode card still gets a blind review panel: critical_only (default, cards marked critical), always, or never (only reviewMode=panel cards).', body: { name: 'Acme AI', slug: 'acme-ai', mission: 'Build useful things.', bossRolePrompt: 'Optional additive Boss role instructions, null or at most 8000 characters; never overrides platform gates.', nfsShareUrl: 'nfs://nas/megacorps optional, informational share location', maxChildrenPerCard: '1-5, default 3: live child cards an agent may open per card', panelReviewDefault: 'critical_only | always | never (default critical_only)', dispatchIntervalSeconds: 10, autoDispatchEnabled: false } },
  { method: 'PUT', path: '/api/companies/:id', group: 'Companies', auth: 'session', summary: 'Update company settings, including panelReviewDefault (critical_only | always | never) for blind review panels.', params: { id: 'Company UUID.' }, body: { name: 'Acme AI', slug: 'acme-ai', mission: 'Updated mission.', bossRolePrompt: 'Optional additive Boss instructions, null or at most 8000 characters; platform gates remain required.', nfsShareUrl: 'nfs://nas/megacorps | null', maxChildrenPerCard: '1-5', panelReviewDefault: 'critical_only | always | never', dispatchIntervalSeconds: 30, autoDispatchEnabled: true } },
  { method: 'GET', path: '/api/companies/:id/deletion-preview', group: 'Companies', auth: 'session', requiredRole: 'admin', summary: 'Read exact blocking counts and related row IDs using the live FK catalog and logical links. Includes archived records; the same inventory is rechecked transactionally before deletion.', params: { id: 'Company UUID.' } },
  { method: 'GET', path: '/api/companies/:id/retirement-preview', group: 'Companies', auth: 'session', requiredRole: 'admin', summary: 'Preview explicit retirement of the legacy Default Company into another company. Requires active admin membership in both. Lists complete blockers, target name/slug conflicts, unbound settings and memberships that will be removed; credentials and repository URLs are omitted.', params: { id: 'Legacy source company UUID.' }, query: { targetCompanyId: 'Required distinct destination company UUID.' } },
  { method: 'POST', path: '/api/companies/:id/retirement', group: 'Companies', auth: 'session', requiredRole: 'admin', summary: 'Explicitly retire the legacy Default Company after a locked complete inventory recheck. Moves only unbound project/runtime definitions, preserves audit globally, and removes unused seeded positions and source memberships. Repository URLs, credentials and policy stay unchanged; derived merge readiness is invalidated and normal target-agent access/readiness checks are required. No repository/provider action or target privilege grant occurs. Same-target retries require the original active actor, current target admin membership and a complete durable success audit. Conflicts return 409 without writes.', params: { id: 'Legacy source company UUID.' }, body: { targetCompanyId: 'Required distinct destination company UUID.' } },
  ...
  { method: 'POST', path: '/api/departments', group: 'Departments', auth: 'session', summary: 'Department headAgentId is derived from the active occupant of its explicit Head Position; omit it when creating a department. Assign Agents through PUT /api/agents/:id with positionId; their department is derived from that Position. description is the department charter.', body: { companyId: 'uuid', name: 'Engineering', slug: 'engineering', headRolePrompt: 'Optional additive head role instructions, null or at most 8000 characters.', headAgentId: 'Derived; omit when creating.', description: 'What this department is responsible for.' } },
  { method: 'PUT', path: '/api/departments/:id', group: 'Departments', auth: 'session', summary: 'Update a department name, charter or head role prompt. headAgentId is derived; change the occupant of its explicit Head Position to change leadership.', params: { id: 'Department UUID.' }, body: { name: 'IT', headRolePrompt: 'Optional additive head role instructions, null or at most 8000 characters.', headAgentId: 'Derived; only the current derived value is accepted.', description: 'What this department is responsible for.' } },
  ...
  { method: 'POST', path: '/api/positions', group: 'Positions', auth: 'session', summary: 'Create a company-scoped Position. Boss has rank 0, implies isCompanyLeadership, and has no department/manager. Head has rank 1, one definition per department, and derives managerPositionId from the active Boss Position. Staff ranks are 2–9: department Staff require defaultDepartmentId; company-direct Staff use isCompanyLeadership=true and no department, without Boss authority. managerPositionId restricts eligible supervisors to its active same-company occupants.', body: { companyId: 'uuid', name: 'CTO', slug: 'cto', description: 'Own technical direction.', reviewDomain: 'code | content | ... optional: the domain this position scores reviews under (feeds the agent CV); empty means general', rank: 1, isCompanyBoss: false, isDepartmentHead: true, isCompanyLeadership: false, defaultDepartmentId: 'department uuid | null', managerPositionId: 'position uuid | null; derived for Head', prompt: 'Own technical direction, architecture decisions, escalation style, and limits.' } },
  { method: 'PUT', path: '/api/positions/:id', group: 'Positions', auth: 'session', summary: 'Update a Position. Assigned departments follow its scope. Changing managerPositionId preserves eligible supervisors, selects a sole candidate for active Agents, or clears the supervisor to an explicit unconfigured state if vacant/ambiguous. Agent Position assignment with multiple candidates instead requires an explicit eligible bossId (organization_supervisor_choice_required). Invalid non-null supervisors return organization_supervisor_ineligible. Company identity is immutable.', params: { id: 'Position UUID.' }, body: { name: 'CTO', slug: 'cto', description: 'Updated description', reviewDomain: 'code | content | null', rank: 1, isCompanyBoss: false, isDepartmentHead: true, isCompanyLeadership: false, defaultDepartmentId: 'department uuid | null', managerPositionId: 'position uuid | null', prompt: 'Updated role-specific prompt.', isActive: true } },
  ...
  { method: 'POST', path: '/api/cards', group: 'Kanban', auth: 'session', summary: 'forceBrainstorm requires a brainstorm round before any split; brainstormDepartmentIds is the floor of departments the client wants consulted. A plain-language goal needs no manual assignee or reviewer: omitted values use strategy-first structural assignment and the existing structural review policy. A named reviewer must differ from the executor. requiresApproval=true adds a separate client approval gate; it is not inferred from an agent reviewer. parentCardId must name a same-company parent and creates a child card under that parent (human splits are unbounded but the parent waits on its children); agents split through report.children instead, bounded by the org-shaped split rules. decisionMode is auto | solo | pair | swarm (legacy values map to these). reviewMode=panel, or critical=true under the company default panelReviewDefault=critical_only, sends the finished work to a blind review panel of two reviewers (reviewerIds names them, max 2; empty means the server composes the panel from the org chart: named reviewers, the department head, same-domain reviewers, the boss chain; the CEO never sits on it) whose findings are merged, answered by the author with dispositions, verified by the same reviewers, and handed up the boss chain when the fix is exhausted; a requiresApproval card then waits for the client as the last gate. The body may be an ordinary natural-language goal. A structured brief with an Acceptance section is optional for human-created cards; generated execution children still require meaningful Acceptance criteria. Create a Kanban task. New tasks default to todo. departmentId/projectId/goalId determine the company, department, project, and selected-goal prompt context.', body: { companyId: 'uuid optional', departmentId: null, projectId: null, goalId: null, title: 'Task title', body: 'Plain-language goal or optional structured brief', priority: 'normal', tags: ['backend'], assigneeId: null, reviewerId: null, coordinationOnly: 'Optional operator-only strategy/coordination exemption from structural delegation, default false. Never bypasses evidence, review, approval or repository gates; agent output and chat actions cannot set it.', requiresApproval: false, reviewMode: 'single | panel (default single)', critical: 'boolean (default false): touches persisted data or performs an irreversible external action', reviewerIds: ['reviewer agent uuid (max 2, panel only)'], decisionMode: 'auto | solo | pair | swarm optional (default auto)', parentCardId: 'uuid | null: create a child card under an existing parent', dependencyCardIds: ['card uuid'], forceBrainstorm: false, brainstormDepartmentIds: ['department uuid'], maxRetries: 3, requiredToolIds: ['tool uuid'], timeoutSeconds: 'optional 30-14400: overrides the agent default and the global Kanban timeout', scheduleAt: 'optional ISO datetime', recurEveryMinutes: 'optional 5-43200', requiredChildPolicy: 'all_required_accepted | all_non_cancelled_accepted | threshold | manual', childRequirementLevel: 'required | optional | follow_up', estimatedWeight: 1, estimatedDurationMinutes: 30, taskBudgetLimit: 2.5, maxRevisions: 3 } },
  { method: 'PUT', path: '/api/cards/:id', group: 'Kanban', auth: 'session', summary: 'Update a Kanban task. Include updatedAt for optimistic locking. Goal selection is validated against the card company, department, and project. An agent collaboration request is supported only from the original card owner dispatch; reviewer, message, or delegated same-card contexts must return to that owned-card route and retry with an actionable request. Independent deliverables are child cards (agents open direct-report work through report.children, humans through POST /api/cards with parentCardId). waiting_on_client and waiting_on_brainstorm are entered by checkpoints and brainstorm rounds, not by hand.', params: { id: 'Task UUID.' }, body: { title: 'Updated title', body: 'Updated detail', columnStatus: 'todo | in_progress | in_review | needs_review | waiting_on_external | waiting_on_client | waiting_on_brainstorm | done | blocked | cancelled', rollupStatus: 'planning | delegated | waiting_on_children | waiting_on_dependencies | waiting_on_external | waiting_on_client | waiting_on_brainstorm | integrating | ready_for_review | done | blocked | null', assigneeId: 'uuid | null', reviewerId: 'uuid | null (an agent, never the assignee)', coordinationOnly: 'Optional operator-only strategy/coordination exemption from structural delegation, default false. Never bypasses evidence, review, approval or repository gates; agent output and chat actions cannot set it.', requiresApproval: true, reviewMode: 'single | panel', critical: true, reviewerIds: ['reviewer agent uuid (max 2)'], departmentId: 'uuid | null', projectId: 'uuid | null', goalId: 'uuid | null', priority: 'urgent | high | normal | low', tags: ['backend'], dependencyCardIds: ['card uuid'], parentCardId: 'uuid | null', decisionMode: 'auto | solo | pair | swarm | null', forceBrainstorm: false, brainstormDepartmentIds: ['department uuid'], maxRetries: 3, requiredToolIds: ['tool uuid'], timeoutSeconds: '30-14400 | null', scheduleAt: 'ISO datetime | null', recurEveryMinutes: '5-43200 | null', requiredChildPolicy: 'all_required_accepted | all_non_cancelled_accepted | threshold | manual', childRequirementLevel: 'required | optional | follow_up', estimatedWeight: 1, estimatedDurationMinutes: 30, taskBudgetLimit: 2.5, revisionCount: 1, maxRevisions: 3, updatedAt: 'ISO datetime from existing card' } },
  { method: 'POST', path: '/api/cards/:id/cancel', group: 'Kanban', auth: 'session', summary: 'Cancel local orchestration without archiving history. Releases local execution locks and cancels queued/running task-runs. An accepted A2A task may still run remotely; its agent remains unavailable until read-only reconciliation confirms natural completion. No remote force-stop is performed.', params: { id: 'Task UUID.' }, body: { reason: 'Optional cancellation reason.' } },
  ...
  { method: 'GET', path: '/api/agent-runtimes/health', group: 'Agents', auth: 'session', summary: 'Read configured runtime state, attached agent counts, and last observed run. status=ready is a legacy configuration/observation summary, never proof of reachability. This endpoint performs no network or model probe; reachability is not_checked.' },
  ...
  { method: 'POST', path: '/api/runner/task-runs/claim', group: 'Runner Agent API', auth: 'runner', summary: 'Successful claims include bounded current companyContext (role/responsibilities/custom role prompts and selected knowledge); prepend it to the runtime task and refresh it on each claim. Atomically claim the highest-priority queued task-run matching runner company and supported runtime.', body: { kinds: ['dispatch', 'review'] } },
  { method: 'POST', path: '/api/runner/task-runs/:id/complete', group: 'Runner Agent API', auth: 'runner', summary: 'Accepts optional structured report and normalized workProducts; required children, review and merge evidence still gate Done. Malformed dispatch/review replies receive one same-session correction, one fresh-context retry, then one eligible department help request or an actionable blocker; transport retry is independent. Complete a runner-claimed task-run and attach work products such as branches, commits, PRs, reports, screenshots, or artifacts. Dispatch success/done queues quality review when the card has a distinct reviewer; review success approves to done. status=waiting_on_external releases the run lock while CI/CD, deploy, export, or outside approval continues, with optional pollIntervalSeconds chosen by the agent/runtime.', params: { id: 'Task-run UUID.' }, body: { status: 'success | failed | cancelled | done | blocked | needs_review | in_review | waiting_on_external', summary: 'Short result', output: 'Full output/log', costUsd: 0.01, pollIntervalSeconds: 'Optional 30-86400 seconds when status=waiting_on_external.', workProducts: [{ type: 'pull_request', title: 'PR #42', pullRequestUrl: 'https://github.com/org/repo/pull/42', branch: 'megacorps/card-1234-alice' }] } },
  ...
  { method: 'GET', path: '/api/chat/sessions/:id/jobs', group: 'Chat', auth: 'session', summary: 'Read the latest 20 durable reply jobs for a session, newest first. Requires viewer access to the session company. Pending jobs survive server and browser restart.', params: { id: 'Chat session UUID.' } },
  { method: 'GET', path: '/api/chat/sessions/:id/jobs/:jobId', group: 'Chat', auth: 'session', summary: 'Read one durable reply job in its authorized session. Status is queued, running, completed or failed; error is present for failures. Never resubmit the user message to poll a reply.', params: { id: 'Chat session UUID.', jobId: 'Chat job UUID.' } },
  ...
  { method: 'GET', path: '/api/projects/:id/merge-readiness', group: 'Projects', auth: 'session', summary: 'Read-only live managed merge readiness: provider version, service identity, ordinary-agent effective permissions and default branch protection. Returns ready, issues, checkedAt, serviceIdentity and project policy; never establishes protection. Dispatch requires ready managed protection. Explicit create/opt-in can establish an absent rule; existing rules are preserved or reported unsafe.', params: { id: 'Project UUID.' } },
  { method: 'GET', path: '/api/cards/:id/merge-intents', group: 'Cards', auth: 'session', summary: 'Inspect durable exact-head merge authorization and bounded attempt state. prepared/retryable may still be prevented by changed gates; in_flight/accepted/uncertain mean the provider may already have accepted the merge and gate changes return 409 until provider reconciliation establishes a terminal result. HTTP acceptance alone never completes a card. Gitea 1.22 binds the head atomically but has no expected-base request field. Managed readiness therefore requires an older case-sensitive literal default glob (main becomes [m]ain) admitting only the verified service and a newer ** fallback denying all nondefault merges, including service merges into nested work branches. Normal work-branch append pushes remain allowed; protected branches forbid force pushes and deletion. Readback must confirm valid created_at timestamps with default strictly earlier at provider-second precision. Plain exact rules are unsafe because Gitea uses case-insensitive matching; existing, overlapping, tied or reversed rules require administrator review and are preserved. Automatic policy supports default names matching /^[A-Za-z0-9_][A-Za-z0-9_./-]*$/; other branch names remain unchanged and unready for opt-in. Later administrator policy changes are not continuously policed; unexpected external effects remain unverified and cannot be cancelled retroactively.', params: { id: 'Card UUID.' } },
  { method: 'POST', path: '/api/projects', group: 'Projects', auth: 'session', summary: 'repoProvider gitea-local auto-provisions an org and repo on the built-in Gitea (with every active company agent added as a write collaborator and a push webhook back to MegaCorps) and fills repoUrl with the clone URL. publishRepoUrl/publishToken store an optional publish target (e.g. GitHub Pages) that is injected into task prompts; the agent performs the publishing push itself. Create a project with optional repo binding and project-level work path. repoUrl is the shared Git truth; workPath is the repo/workspace-relative area agents should edit; each remote agent still uses its own runtime-local clone. Agent workspaces live on the company NFS mount at {mount}/{companySlug}/agents/{agentSlug}/project/{projectSlug}/ when configured. completionRequiresMerge requires provider-verified repository, target branch and full reviewed head evidence; missing or stale evidence blocks completion with a correction request. A pull_request work product accepts url or pullRequestUrl. It turns on the merge gate (design §19): an approved card parks on waiting_on_external with the exact head SHA the review authorized and only reaches done when that commit lands on defaultBranch. It defaults to true for a new gitea-local project and false everywhere else; send it explicitly to override.', body: { companyId: 'uuid optional', name: 'Project name', description: 'Optional description', repoProvider: 'github', repoUrl: 'https://github.com/org/repo', workPath: 'apps/server or reports/final, null means project root', defaultBranch: 'main', protectedBranches: ['main', 'master'], workBranchPattern: 'megacorps/card-{cardId}-{agentSlug}', pullBeforeRun: true, pushAfterRun: true, completionPolicy: 'push_or_pr', completionRequiresMerge: 'optional boolean; defaults to true for gitea-local, false otherwise', autoMergeAfterApproval: 'optional boolean; defaults true only for newly provisioned managed Gitea repos with merge completion enabled; explicit opt-in verifies protection and stores readiness; legacy default false', setupCommand: 'npm install', testCommand: 'npm test', runtimeServices: { web: 'http://localhost:3000' }, workspacePathHint: 'optional runtime-local clone/folder hint only', publishRepoUrl: 'https://github.com/org/site optional publish target', publishToken: 'optional token injected into task prompts for the publish push' } },
  { method: 'PUT', path: '/api/projects/:id', group: 'Projects', auth: 'session', summary: 'Update a project repo binding, project work path, branch policy, runtime services, setup/test commands, or description. Accepts any subset of the POST /api/projects fields. Contending gate writes retry the complete database operation up to five times; an exhausted retry returns 409 merge_gate_write_busy with the database operation rolled back. Provider setup calls are never replayed by that database retry.', params: { id: 'Project UUID.' }, body: { name: 'optional', description: 'optional', repoProvider: 'github | gitlab | gitea | gitea-local | generic', repoUrl: 'optional', workPath: 'optional', defaultBranch: 'optional', protectedBranches: ['main'], workBranchPattern: 'optional', pullBeforeRun: true, pushAfterRun: true, completionPolicy: 'push_branch | pull_request | push_or_pr | manual', completionRequiresMerge: 'optional boolean; true parks approved cards until the authorized head is merged into defaultBranch', autoMergeAfterApproval: 'optional boolean; explicit managed Gitea opt-in establishes safe protection when absent, preserves existing rules, and reports readiness; false preserves manual merging', setupCommand: 'optional', testCommand: 'optional', runtimeServices: {}, workspacePathHint: 'optional', publishRepoUrl: 'optional', publishToken: 'optional' } },
  ...
  { method: 'GET', path: '/api/cron/status', group: 'Cron', auth: 'session', requiredRole: 'viewer', summary: 'Read dispatch loop status plus ten recent run summaries without details.' },
  { method: 'GET', path: '/api/cron/runs', group: 'Cron', auth: 'session', requiredRole: 'viewer', summary: 'Read cron/heartbeat loop runs. view=summary excludes details and returns {items,nextCursor}; use the detail route for one run.', query: { view: 'summary opts into metadata pagination; omitted preserves the legacy array.', q: 'Server-side name/source/status/error/details search, max 200 characters.', limit: 'Summary: 1-100, default 50. Legacy: 1-500, default 100.', cursor: 'Opaque exact createdAt+id cursor; summary mode only.' } },
  { method: 'GET', path: '/api/cron/runs/:id', group: 'Cron', auth: 'session', requiredRole: 'viewer', summary: 'Read one cron run including details.', params: { id: 'Cron run UUID.' } },
...
const currentArchitecture = {
  ...
  sourceOfTruth: [
    ...
    'Knowledge: company-scoped markdown docs. Empty tags or general apply company-wide; other tags match task tags or department names/slugs, ignoring case. Policy and Handbook are ordinary tags, not automatic company-wide rules. Selected content is bounded by prompt size limits.',
    ...
    'External Events: waiting_on_external cards release execution locks while CI/CD, deployment, exports, or outside approvals complete; external events wake cards into review, rework, done, or blocked. Agents/runners can provide pollIntervalSeconds so external polling cadence is task-specific rather than tied to the dispatch heartbeat: the poll sweep re-dispatches the owner for a check-only turn at that cadence, bounded at 24 checks per wait, and stops with an external_poll_exhausted note when the budget runs out.',
  ...
  surfaces: [
    ...
    { name: 'Knowledge', route: '/knowledge', purpose: 'Company-scoped markdown knowledge: empty/general tags apply company-wide; other tags match task or department context, ignoring case, within prompt size limits.', primaryApi: ['/api/knowledge-docs'] },
  ...
  multiAgentNotes: [
    ...
    'Use waiting_on_external when a card has produced a PR/deploy/export and must wait for outside CI/CD or approval; do not keep execution locks while waiting. Include pollIntervalSeconds when the external system will not call back; MegaCorps then wakes you at that cadence to look, and re-reporting waiting_on_external updates the same wait instead of stacking a new one.',
    ...
    'Use in_review for completed work that still needs quality review; webhook and runner completion do not bypass a distinct reviewer. Use needs_review for reviewer guidance when an assignee cannot complete a task; if an independent reviewer/manager exists MegaCorps queues help review, otherwise the card parks in_review for human approval and is never auto-closed as done.',
...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  const usageExample = { actualUsd: '0.00000019', estimatedUsd: '0.25000000', totalUsd: '0.25000019', reservedUsd: '1.00000000', unknownAttempts: 1, attempts: 3, period: { key: '2026-09', start: '2026-09-01T00:00:00.000Z', end: '2026-10-01T00:00:00.000Z', timezone: 'UTC' } };
  if (endpoint.path === '/api/usage-summary') return {
    responseSchema: { actualUsd: 'decimal string, 8 places', estimatedUsd: 'decimal string, 8 places', totalUsd: 'known subtotal, not total billing', reservedUsd: 'current active exposure across all periods', unknownAttempts: 'integer', attempts: 'integer', period: 'UTC half-open month interval or null (all)', reservationAsOf: 'ISO datetime', accounting: 'string', taskScope: 'string' },
    responseExample: { ...usageExample, reservationAsOf: '2026-09-06T00:00:00.000Z', accounting: 'Runtime-reported actual, estimates and unknown usage; not invoice reconciliation.', taskScope: 'Direct card executions; child cards accounted independently.' }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint),
  };
  if (endpoint.path === '/api/cost-events') return {
    responseSchema: { type: 'array', items: { id: 'uuid', companyId: 'uuid', agentId: 'original uuid', cardId: 'original uuid|null', projectId: 'original uuid|null', attemptKey: 'immutable string|null (legacy)', taskRunId: 'uuid|null', heartbeatRunId: 'uuid|null', runtimeId: 'original uuid|null', reportingSource: 'immutable reporting namespace|null (legacy)', providerEventId: 'stable provider event|null', source: 'entrypoint or legacy_fixed_rate', costUsd: 'decimal string|null (unknown)', costStatus: 'actual|estimated|unknown', usage: 'whitelisted usage facts|null (legacy)', occurredAt: 'ISO datetime', reservationUsd: 'decimal string|null', reservationExpiresAt: 'ISO datetime|null', settledAt: 'ISO datetime|null' } },
    responseExample: [{ id: 'event-uuid', companyId: 'company-uuid', agentId: 'agent-uuid', attemptKey: 'task-run:run-uuid', source: 'dispatch', runtimeId: 'runtime-uuid', reportingSource: 'company-uuid:runtime:runtime-uuid', costUsd: null, costStatus: 'unknown', usage: { costStatus: 'unknown', tokenStatus: 'estimated', source: 'character_count_output', costUsd: null, totalTokens: 100, provider: null, model: null } }], rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint),
  };
  ...
  if (endpoint.path === '/api/runner/task-runs/claim') {
    return { responseSchema: { taskRun: 'TaskRun | null (no eligible work)', card: 'Card, only on successful claim', agent: 'Redacted agent, only on successful claim', project: 'Project | null, only on successful claim', runtime: 'Redacted runtime | null, only on successful claim', companyContext: 'Bounded current prompt string including exact pre-review identity, only on successful claim', reviewIdentity: 'Server-captured repository/base/full-head snapshot or null; immutable on taskRun and required for merge approval' }, responseExample: { taskRun: { id: 'task-run-uuid', status: 'running' }, card: { id: 'card-uuid' }, agent: { id: 'agent-uuid' }, project: null, runtime: null, reviewIdentity: null, companyContext: 'Current company role and selected knowledge context.' }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  if (endpoint.path === '/api/runner/task-runs/:id/complete') {
    return { responseSchema: { success: 'Updated Card row; ID is card ID, not task-run ID', duplicate: { ok: 'boolean', duplicate: true, taskRunId: 'uuid', status: 'stored run status' }, notes: 'Normal card transition remains subject to report normalization, evidence/review/child/merge gates. Duplicate acknowledgment does not imply Done.' }, responseExample: { id: 'card-uuid', columnStatus: 'in_review' }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  if (endpoint.path === '/api/companies/:id/execution-readiness') {
    const readiness = { ready: false, structureReady: false, issues: ['Assign exactly one company Boss position to an agent.'], setupIssues: [], runtimeIssues: [], repositoryWriteAccess: 'not_required', repositoryIssues: [] };
    return { responseSchema: { ready: 'boolean; admission only, not a live model probe', structureReady: 'boolean', issues: 'blocking string[]', setupIssues: 'nonblocking draft string[]', runtimeIssues: 'string[]', repositoryWriteAccess: 'not_required | not_checked | ready | blocked', repositoryIssues: 'string[]' }, responseExample: readiness, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  if (endpoint.path === '/api/company-setup' || endpoint.path === '/api/companies/:id/setup') {
    const state = { company: { id: 'company-uuid', autoDispatchEnabled: false }, draft: {}, boss: null, department: null, head: null };
    return { responseSchema: { company: 'Company', draft: 'persisted setup progress', boss: 'public agent identity | null', department: 'Department | null', head: 'public agent identity | null', ...(endpoint.method === 'GET' ? { readiness: 'structural/runtime admission', connectionIssues: 'string[]', status: 'draft | ready | needs_attention | dispatch_disabled' } : {}) }, responseExample: endpoint.method === 'GET' ? { ...state, readiness: { ready: false, issues: [] }, connectionIssues: [], status: 'draft' } : state, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  if (endpoint.path === '/api/companies/:id/setup/probe') return { responseSchema: { results: 'Array<{agentId, success, message}>; explicit A2A discovery, not model execution' }, responseExample: { results: [] }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  ...
  if (endpoint.path.includes('/api/admin/users')) {
    const user = { id: 'user-uuid', email: 'admin@example.com', name: 'Admin', role: 'admin', status: 'active', memberships: [{ companyId: 'company-uuid', companyName: 'Example Company', role: 'admin', status: 'active' }] };
  ...
  if (endpoint.path.includes('/projects')) {
    const project = { id: 'project-uuid', companyId: 'company-uuid', name: 'Project name', repoProvider: 'github', repoUrl: 'https://github.com/org/repo', workPath: 'apps/server', defaultBranch: 'main', workBranchPattern: 'megacorps/card-{cardId}-{agentSlug}', completionRequiresMerge: false, autoMergeAfterApproval: false, managedRepoFullName: 'org/repo | null', mergeReadiness: 'stored inspection result, not current worker push capability', publishToken: '[redacted] | null', workspacePathHint: 'optional runtime-local hint only' };
  ...
  if (endpoint.path === '/api/agent-runtimes/health') {
    const health = { runtimeId: 'runtime-uuid', name: 'Configured A2A', adapterType: 'a2a', status: 'ready', statusBasis: 'configuration_and_observed_runs', reachability: 'not_checked', isActive: true, agents: 0, activeAgents: 0, busyAgents: 0, lastRunAt: null, lastRunStatus: null, lastError: null, capabilities: ['a2a', 'json-rpc', 'task-push-notifications'] };
    return { responseSchema: { type: 'array', items: { ...health, status: 'ready | busy | degraded | disabled; legacy configured/observed summary only', lastRunAt: 'ISO datetime | null', lastRunStatus: 'string | null', lastError: 'string | null' } }, responseExample: [health], rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  ...
  if (endpoint.path.includes('/task-runs') || endpoint.path.endsWith('/run') || endpoint.path.endsWith('/review')) {
    ...
    if (endpoint.method === 'GET' && endpoint.path.includes(':id')) return { responseSchema: { ...taskRun, output: 'string | null' }, responseExample: { ...taskRun, output: 'Full run output.' }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
    if (endpoint.method === 'GET') return { responseSchema: { legacy: { type: 'array', items: { ...taskRun, output: 'string | null' } }, summary: { items: [{ ...taskRun, preview: 'bounded string | null' }], nextCursor: 'opaque string | null' }, errors: ['400 invalid_limit', '400 invalid_cursor', '400 invalid_search'] }, responseExample: { items: [taskRun], nextCursor: null }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  ...
  if (endpoint.path.includes('/prompt-logs')) {
    ...
    if (endpoint.path.includes(':id')) return { responseSchema: promptLog, responseExample: promptLog, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
    const summary = { id: promptLog.id, companyId: promptLog.companyId, agentId: promptLog.agentId, title: promptLog.title, preview: 'Bounded prompt preview.', contextMode: 'full_bootstrap', createdAt: promptLog.createdAt };
    return { responseSchema: { legacy: { type: 'array', items: promptLog }, summary: { items: [summary], nextCursor: 'opaque string | null' }, errors: ['400 invalid_limit', '400 invalid_cursor', '400 invalid_search'] }, responseExample: { items: [summary], nextCursor: null }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }

  if (endpoint.path.endsWith('/cancel')) {
    return { responseSchema: { type: 'card', id: 'uuid', columnStatus: 'cancelled', updatedAt: 'ISO datetime', remoteWork: 'null | { pending, status, count, reason }' }, responseExample: { id: 'card-uuid', columnStatus: 'cancelled', lastError: 'Cancelled by operator.' }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }

  if (endpoint.path.includes('/chat/sessions/:id/jobs')) {
    const job = { id: 'job-uuid', sessionId: 'session-uuid', userMessageId: 'message-uuid', status: 'queued | running | completed | failed', error: 'string | null', createdAt: 'ISO datetime', updatedAt: 'ISO datetime' };
    return { responseSchema: endpoint.path.endsWith('/:jobId') ? job : { type: 'array', items: job }, responseExample: endpoint.path.endsWith('/:jobId') ? { ...job, status: 'running', error: null } : [{ ...job, status: 'running', error: null }], rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }

  if (endpoint.path.includes('/chat/sessions/:id/messages')) {
    const message = { id: 'message-uuid', sessionId: 'session-uuid', authorType: 'user | agent | system', body: 'Message body', createdAt: '2026-06-06T00:00:00.000Z' };
    ...
    return { responseSchema: { accepted202: { session: 'ChatSession', userMessage: message, job: { id: 'uuid', sessionId: 'uuid', userMessageId: 'uuid', status: 'queued | running', error: null } }, synchronous200: { session: 'ChatSession', userMessage: message, agentMessage: message } }, responseExample: { session: { id: 'session-uuid' }, userMessage: { ...message, authorType: 'user' }, job: { id: 'job-uuid', sessionId: 'session-uuid', userMessageId: 'message-uuid', status: 'queued', error: null } }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  ...
  if (endpoint.path === '/api/cron/runs/:id') {
    return { responseSchema: { id: 'uuid', name: 'string', status: 'string', details: 'object', createdAt: 'ISO datetime' }, responseExample: { id: 'cron-run-uuid', name: 'dispatch-heartbeat', status: 'success', details: {}, createdAt: '2026-06-06T00:00:00.000Z' }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }

  if (endpoint.path === '/api/cron/runs' && endpoint.method === 'GET') {
    return { responseSchema: { legacy: { type: 'array', items: 'CronRun with details' }, summary: { items: 'CronRun[] without details', nextCursor: 'opaque string | null' }, errors: ['400 invalid_limit', '400 invalid_cursor', '400 invalid_search'] }, responseExample: { items: [], nextCursor: null }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  ...
  if (endpoint.method === 'GET' && ['/api/system-logs', '/api/activity', '/api/admin/activity', '/api/heartbeat-runs'].includes(endpoint.path)) {
...
function endpointWithDefaults(endpoint: ApiEndpoint): ApiHelpEndpoint {
  ...
  if (endpoint.path === '/api/webhook/task-complete') endpoint = { ...endpoint, notes: [...(endpoint.notes ?? []), 'A current authenticated review callback with outer needs_review and a structured completed/approved report returns 409 review_status_conflict before usage or completion effects. Correct the same run with outer done or in_review plus completed/approved; actual assistance uses structured input_required with request.kind=help or an explicit verdict=escalate. Permission/failure stops, explicit waits, existing client gates and reportless guidance retain their existing behavior.'] };
  if (endpoint.path.includes('/retirement')) endpoint = { ...endpoint, response: '200 preview or committed retirement result. 400 validation_failed/company_retirement_same_target; 401 auth_required; 403 company_role_required; 404 company_not_found; 409 company_retirement_blocked/company_retirement_busy or incompatible legacy-source/replay state. Source and target both require current admin membership; global admin role alone is insufficient.' };
  if (endpoint.path === '/api/agents' || endpoint.path === '/api/agents/:id') endpoint = { ...endpoint, notes: [...(endpoint.notes ?? []), 'spentThisMonth is a compatibility cache, not the current accounting source. Use GET /api/usage-summary for actual/estimated/unknown/current reservations and explicit UTC periods. PUT accepts explicit null to clear hermesProfile, budgetPerTask and budgetMonthly. Omitted fields preserve stored values; hidden adapter/runtime fields are not reset by organization-only updates.'] };

  if (endpoint.method === 'GET' && /^\/api\/(system-logs|prompt-logs|activity|admin\/activity|heartbeat-runs|task-runs|cron\/runs)(\/:id)?$/.test(endpoint.path)) endpoint = { ...endpoint, notes: [...(endpoint.notes ?? []), 'Detail IDs and supported company/card/agent UUID filters are validated before SQL; malformed IDs return 400 validation_failed with field issues. Missing or out-of-scope details return 404 log_not_found; global-admin routes return 403 for insufficient role before reading rows. Cursor/date errors retain 400 invalid_cursor.'] };

  if (endpoint.path === '/api/company-setup') endpoint = { ...endpoint, body: z.toJSONSchema(startInput, { io: 'input' }), response: '201 persisted setup state (including an idempotent retry); 400 validation_failed, 401 auth_required, 403 forbidden, 409 setup_slug_taken.' };
  if (endpoint.path === '/api/companies/:id/setup' && endpoint.method === 'PUT') endpoint = { ...endpoint, body: z.toJSONSchema(stepInput, { io: 'input' }), response: '200 persisted setup state; 400 validation_failed or invalid setup relation/runtime, 401 auth_required, 403 company_access_denied, 409 setup_not_ready or changed runtime creation request.' };
  if (endpoint.path === '/api/runner/task-runs/claim') endpoint = { ...endpoint, body: z.toJSONSchema(runnerTaskClaimSchema, { io: 'input' }), notes: [...(endpoint.notes ?? []), '200 {taskRun:null} means no eligible work. Successful claim includes the full envelope and current companyContext; 401 invalid runner key, 403 runner_company_mismatch. The repository CLI saves the whole claim in a manifest and creates a worktree; it does not execute AI or consume the prompt. External executors must prepend and refresh companyContext themselves.'] };
  if (endpoint.path === '/api/runner/task-runs/:id/complete') endpoint = { ...endpoint, body: { schema: z.toJSONSchema(runnerTaskCompleteSchema, { io: 'input' }), report: z.toJSONSchema(agentReportSchema, { io: 'input' }) }, notes: [...(endpoint.notes ?? []), 'Canonical report states: completed, progress, input_required, failed, rejected. Deprecated in_progress normalizes to progress. request is singular: permission, help or checkpoint. Present malformed reports receive bounded correction, never product approval. 401 runner authentication required; 404 task_run_not_found for absent/foreign company; 409 task_run_not_claimed_by_runner for wrong owner.'] };
  ...
  if (endpoint.path === '/api/cost-events') endpoint = { ...endpoint, notes: [...(endpoint.notes ?? []),
    ...
    'Progress usage remains unsettled. reservationUsd is outstanding exposure in addition to booked cumulative cost; a cumulative cost change adjusts the remaining reservation by its delta, floored at zero. Duplicate progress does not reduce exposure again or extend reservationExpiresAt. Terminal provider return, authenticated terminal callback or explicit cancellation clears the reservation; bounded expiry excludes it from current exposure. Late progress retains facts without reopening a terminal reservation.',
  ] };
  if (['/api/webhook/task-complete', '/api/runner/task-runs/:id/complete', '/api/a2a/push'].includes(endpoint.path)) endpoint = { ...endpoint, notes: [...(endpoint.notes ?? []),
    'Optional usage version 1 fields: costStatus and tokenStatus (actual|estimated|unknown), costUsd (nonnegative decimal string with up to 8 places, or finite number), provider/model when reported, inputTokens/outputTokens/cacheReadTokens/cacheWriteTokens/reasoningTokens/totalTokens (nonnegative safe integers), providerEventId (stable within original reporting source and provider), occurredAt (ISO timestamp with timezone). Missing facts are unknown; source provenance is assigned by the server. Actual means runtime-reported, not invoice reconciled. Generated answer prose is never billing metadata.',
    'Partial token corrections retain absent/null fields and accept explicitly supplied zero. An empty token declaration cannot upgrade existing estimates. Per-field provenance is assigned by the server, not accepted from client tokenProvenance. Once a provider event is bound, a correction cannot change its provider namespace even when providerEventId is omitted; this also applies to initially unknown providers. Conflicts return 409 and preserve the existing ledger.',
...
export function apiHelpCatalog() {
  ...
  return {
    ...
    agentReporting: {
      ...
      collaboration: 'Owned execution or management card only: Staff or a department Head may return one input_required request with kind collaboration, departmentSlug, question and 1-10 acceptance strings. It directly creates a required child beneath the original card with the target department Head as assignee. There is no preapproval. Staff prefers source Head plus requester as reviewers; Head uses the source Head. A recorded reviewer shortage may downgrade once; busy reviewers wait. Accepted work returns to the original card owner for integration.',
