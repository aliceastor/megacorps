...
const currentArchitecture = {
  ...
  multiAgentNotes: [
    ...
    'Kanban dispatch/review uses full_bootstrap on the first scoped card session and adapter_session_delta on later turns for codex-app and hermes-ssh. The delta refreshes DB truth such as stage, lastError, review feedback, child/dependency state, messages, actions, logs, and work products.',
...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  ...
  if (endpoint.path.includes('/cards/:id/subtree')) {
    return { responseSchema: { type: 'array', items: { id: 'uuid', parentCardId: 'uuid', title: 'string', columnStatus: 'CardStatus', assigneeId: 'uuid | null', reviewerId: 'uuid | null', depth: 'number', childCount: 'number', updatedAt: 'ISO datetime' } }, responseExample: [{ id: 'child-card-uuid', parentCardId: 'card-uuid', title: 'Child task', columnStatus: 'in_progress', assigneeId: 'agent-uuid', reviewerId: null, depth: 1, childCount: 2, updatedAt: '2026-06-09T00:00:00.000Z' }], rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
