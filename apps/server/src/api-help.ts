...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'PUT', path: '/api/cards/:id', group: 'Kanban', auth: 'session', summary: 'Update a Kanban task. Include updatedAt for optimistic locking. Goal selection is validated against the card company, department, and project. Manual recovery can move cancelled tasks to done when an operator accepts preserved output.', params: { id: 'Task UUID.' }, body: { title: 'Updated title', body: 'Updated detail', columnStatus: 'todo | in_progress | in_review | needs_review | waiting_on_external | done | blocked | cancelled', rollupStatus: 'planning | delegated | waiting_on_children | waiting_on_dependencies | waiting_on_external | integrating | ready_for_review | done | blocked | null', projectId: 'uuid | null', goalId: 'uuid | null', decisionMode: 'execute | delegate | hybrid | review | integrate | null', requiredChildPolicy: 'all_required_accepted | all_non_cancelled_accepted | threshold | manual', childRequirementLevel: 'required | optional | follow_up', estimatedWeight: 1, estimatedDurationMinutes: 30, taskBudgetLimit: 2.5, revisionCount: 1, maxRevisions: 3, updatedAt: 'ISO datetime from existing card' } },
