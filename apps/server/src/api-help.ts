...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/companies', group: 'Companies', auth: 'session', summary: 'Create a company.', body: { name: 'Acme AI', slug: 'acme-ai', mission: 'Build useful things.', nfsShareUrl: 'nfs://nas/megacorps optional, informational share location', maxChildrenPerCard: '1-5, default 3: live child cards an agent may open per card', dispatchIntervalSeconds: 10, autoDispatchEnabled: true } },
  { method: 'PUT', path: '/api/companies/:id', group: 'Companies', auth: 'session', summary: 'Update company settings.', params: { id: 'Company UUID.' }, body: { name: 'Acme AI', slug: 'acme-ai', mission: 'Updated mission.', nfsShareUrl: 'nfs://nas/megacorps | null', maxChildrenPerCard: '1-5', dispatchIntervalSeconds: 30, autoDispatchEnabled: true } },
  ...
  { method: 'GET', path: '/api/cards/:id/comments', group: 'Kanban', auth: 'session', summary: 'Read task message board comments, newest first. Peer questions (action=peer_question) and their answers (action=peer_answer) thread through parentCommentId: a question raised by an @mention carries metadata.mention=true plus metadata.sourceCommentId (the comment that mentioned the agent), metadata.authorName and metadata.authorKind (user | agent); the answer has parentCommentId set to the question. Agent comments posted through the bearer route carry metadata.via=agent_token, report notes metadata.via=report.', params: { id: 'Task UUID.' } },
  ...
  { method: 'POST', path: '/api/gitea/events', group: 'Kanban', auth: 'none', summary: 'Push webhook receiver for the bundled Gitea, authenticated by the token query parameter that MegaCorps set when registering the hook. Records the push in the activity log and live feed.', query: { token: 'Webhook token issued at hook registration.' } },
...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  ...
  if (endpoint.path.includes('/cards/:id/comments')) {
    const comment = { id: 'comment-uuid', cardId: 'card-uuid', parentCommentId: 'comment-uuid | null', authorType: 'user | agent | system', authorId: 'user uuid | null', agentId: 'uuid | null', assigneeAgentId: 'uuid | null', reviewerAgentId: 'uuid | null', reviewerScope: 'phase | final | null', delegationStatus: 'queued | running | waiting | submitted | approved | rejected | failed | done | cancelled | null', body: 'Comment body', action: 'comment | agent_note | delegate_request | delegate_report | phase_review_approved | final_review_approved | peer_question | peer_answer | peer_question_failed', metadata: { mention: 'true when raised by an @mention', sourceCommentId: 'comment uuid | null', authorName: 'string', authorKind: 'user | agent', via: 'agent_token | report' }, createdAt: '2026-06-06T00:00:00.000Z' };
