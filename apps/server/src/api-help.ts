...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'GET', path: '/api/companies/:id/setup', group: 'Companies', auth: 'session', requiredRole: 'operator', summary: 'Resume persisted company, Boss, department, head, runtime and connection-check state. Registered runtime metadata is not proof of a live connection.', params: {id:'Company UUID'} },
  { method: 'PUT', path: '/api/companies/:id/setup', group: 'Companies', auth: 'session', requiredRole: 'operator', summary: 'Save a sequential setup step transactionally without duplicate entities. Boss and head are distinct. Finish explicitly enables dispatch only after structure and recent successful connection checks. Repository write and protected-merge readiness remain separate project checks.', params: {id:'Company UUID'}, body:{step:'company | boss | department | head | runtime | finish'} },
