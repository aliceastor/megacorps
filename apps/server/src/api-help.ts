...
const endpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', path: '/api/positions', group: 'Positions', auth: 'session', summary: 'Create a company-scoped position prompt and authority profile. Boss has rank 0 and no department/manager. Department Head has rank 1 and isDepartmentHead=true, with one head position per department including inactive definitions. Staff ranks are 2–9. Every non-Boss position requires defaultDepartmentId.', body: { companyId: 'uuid', name: 'CTO', slug: 'cto', description: 'Own technical direction.', reviewDomain: 'code | content | ... optional: the domain this position scores reviews under (feeds the agent CV); empty means general', rank: 1, isCompanyBoss: false, isDepartmentHead: true, canDelegateAcrossDepartments: false, defaultDepartmentId: 'department uuid | null', managerPositionId: 'position uuid | null', prompt: 'Own technical direction, architecture decisions, escalation style, and limits.' } },
  { method: 'PUT', path: '/api/positions/:id', group: 'Positions', auth: 'session', summary: 'Update a company-scoped position prompt and authority profile. Assigned Agent departments follow defaultDepartmentId; Staff bossId is preserved. The company cannot be changed after creation; deleting the active company boss is blocked.', params: { id: 'Position UUID.' }, body: { name: 'CTO', slug: 'cto', description: 'Updated description', reviewDomain: 'code | content | null', rank: 1, isCompanyBoss: false, isDepartmentHead: true, canDelegateAcrossDepartments: true, defaultDepartmentId: 'department uuid | null', managerPositionId: 'position uuid | null', prompt: 'Updated role-specific prompt.', isActive: true } },
