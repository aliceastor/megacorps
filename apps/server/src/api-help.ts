...
function responseDefaults(endpoint: ApiEndpoint): Pick<ApiHelpEndpoint, 'responseSchema' | 'responseExample' | 'rateLimit' | 'requiredRole'> {
  ...
  if (endpoint.path.includes('/prompt-logs')) {
    ...
    return { responseSchema: { legacy: { type: 'array', items: promptLog }, summary: { items: [summary], nextCursor: 'opaque string | null' }, errors: ['400 invalid_limit', '400 invalid_cursor'] }, responseExample: { items: [summary], nextCursor: null }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  ...
  if (endpoint.path === '/api/cron/runs' && endpoint.method === 'GET') {
    return { responseSchema: { legacy: { type: 'array', items: 'CronRun with details' }, summary: { items: 'CronRun[] without details', nextCursor: 'opaque string | null' }, errors: ['400 invalid_limit', '400 invalid_cursor'] }, responseExample: { items: [], nextCursor: null }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
  ...
  if (endpoint.method === 'GET' && ['/api/system-logs', '/api/activity', '/api/admin/activity', '/api/heartbeat-runs'].includes(endpoint.path)) {
    return { responseSchema: { legacy: { type: 'array', items: entity }, summary: { items: `${entity} metadata[] without body/details`, nextCursor: 'opaque string | null' }, errors: ['400 invalid_limit', '400 invalid_cursor'] }, responseExample: { items: [], nextCursor: null }, rateLimit: endpoint.rateLimit ?? defaultRateLimit, requiredRole: roleDefault(endpoint) };
  }
