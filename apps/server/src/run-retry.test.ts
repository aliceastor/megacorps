...
import { companies, agents, approvals, cardComments, kanbanCards, machineRunners, taskRuns } from './db/schema.ts';
...
for (const kind of ['review', 'message', 'message_review']) {
  test(`${kind} failures back off 1/5/15/30 minutes, then block without queuing more work`, async (t) => {
    ...
    const state = memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [taskRuns, []], [cardComments, [comment]]]);
...
test('ignored reviewer completion settles idempotently without changing the preserved card or retry state', async (t) => {
  ...
  memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [taskRuns, [run]]]);
...
test('success resets only its kind, duplicate completion does not count twice, operator run resets the stop', async (t) => {
  ...
  const state = memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [taskRuns, []]]);
...
test('queue claimant respects delay and blocked cards while another run kind remains eligible', async (t) => {
  ...
  const state = memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [agents, [{ id: 'reviewer', isActive: true, isBusy: false }]], [taskRuns, [{ id: 'queued', cardId: card.id, kind: 'review', status: 'queued' }]]]);
...
for (const kind of ['review', 'message', 'message_review']) {
  test(`machine runner cannot bypass ${kind} delay or exhaustion`, async (t) => {
    ...
    const state = memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [machineRunners, [{ id: 'runner', companyId: 'company', name: 'Runner', apiKeyHash: hashRunnerApiKey('test-only-key') }]], [agents, [{ id: 'agent', companyId: 'company', adapterType: 'webhook' }]], [taskRuns, [{ id: 'queued', companyId: 'company', cardId: card.id, agentId: 'agent', kind, status: 'queued' }]]]);
    const app = Fastify();
    t.after(() => app.close());
...
test('real review and message adapter failures record backoff; a pending human gate survives exhaustion', async (t) => {
  ...
  const state = memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [agents, [reviewer]], [cardComments, [comment]], [taskRuns, [{ id: 'review', companyId: card.companyId, agentId: reviewer.id, cardId: card.id, kind: 'review', status: 'running' }, { id: 'message', companyId: card.companyId, cardId: card.id, kind: 'message', messageCommentId: comment.id, agentId: reviewer.id, status: 'running' }]]]);
  ...
  state.rows(taskRuns).push({ id: 'message-review', companyId: card.companyId, cardId: card.id, kind: 'message_review', messageCommentId: 'report', agentId: reviewer.id, status: 'running' });
...
for (const kind of ['review', 'message', 'message_review']) {
  test(`successful machine runner ${kind} completion resets its streak`, async (t) => {
    ...
    memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [machineRunners, [{ id: 'runner', companyId: 'company', name: 'Runner', apiKeyHash: hashRunnerApiKey('test-only-key') }]], [agents, [{ id: 'original-worker', companyId: 'company' }]], [taskRuns, [{ id: 'run', companyId: 'company', agentId: 'original-worker', cardId: card.id, kind, status: 'running', lockedBy: 'runner' }]]]);
    const app = Fastify();
    t.after(() => app.close());
    await registerRunnerRoutes(app);
...
for (const includeRunId of [true, false]) {
  test(`successful review webhook resets its streak with ${includeRunId ? 'run ID' : 'heartbeat fallback'}`, async (t) => {
    ...
    memoryDb(t, [[companies, [{ id: card.companyId }]], [kanbanCards, [card]], [taskRuns, [{ id: runId, companyId: card.companyId, cardId: card.id, heartbeatRunId: 'heartbeat', kind: 'review', status: 'running' }]]]);
    const app = Fastify();
    t.after(() => app.close());
