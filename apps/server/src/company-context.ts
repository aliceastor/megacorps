...
export async function buildCompanyContextParts(companyId: string, agentId?: string | null, tags: string[] = []) {
  ...
  const roleText = [
    ...
    formatAgentPositionPrompt({ positionName: position?.name, departmentName: department?.name, companyName: structure.company?.name, customPrompt: position?.prompt, isCompanyLeadership: Boolean(position?.isCompanyLeadership || position?.isCompanyBoss), agent, position }),
  ].filter(Boolean).join('\n\n');
