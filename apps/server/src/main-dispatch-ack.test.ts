...
import { A2aTaskRunLease, withA2aRecoveryRun, withA2aTaskRunLease } from './a2a-task-recovery.ts';
import { db } from './db/client.ts';
import { agents, a2aExecutionAliases, a2aExecutions, heartbeatRuns, kanbanCards, taskRuns, workProducts } from './db/schema.ts';
...
function fixture(t: TestContext) {
  ...
  const children: any[] = ['Research', 'Implementation'].map(title => ({ id: randomUUID(), companyId: card.companyId, parentCardId: card.id, title, assigneeId: card.assigneeId, projectId: null, columnStatus: 'done', childRequirementLevel: 'required', deletedAt: null }));
  ...
  agent.isBusy = true;
  const run: any = { id: randomUUID(), companyId: card.companyId, cardId: card.id, agentId: agent.id, heartbeatRunId: heartbeat.id, kind: 'dispatch', status: 'running', lockedBy: 'worker' };
  const products = children.map((child, index) => ({ id: randomUUID(), companyId: card.companyId, cardId: child.id, projectId: null, agentId: agent.id, type: 'report', title: `Accepted child ${index + 1}` }));
  const state = memoryDb(t, [[kanbanCards, [card, ...children]], [agents, [agent]], [heartbeatRuns, [heartbeat]], [taskRuns, [run]], [workProducts, products], [a2aExecutions, []], [a2aExecutionAliases, []]]);
...
test('main dispatch completion consumes its terminal A2A journal before the next same-card execution', async t => {
  ...
  for (const child of state.rows(kanbanCards).filter(row => row.parentCardId === card.id)) child.deliveryAcceptance = await captureDeliveryAcceptance(child as any);
  ...
  await withA2aTaskRunLease(new A2aTaskRunLease(run), () => withA2aRecoveryRun(run, () => dispatchCard(card.id, 'manual', { taskRunId: run.id })));

  assert.equal(run.status, 'success');
  assert.equal(card.columnStatus, 'done');
  assert.ok(card.deliveryAcceptance, 'successful parent integration must seal inherited delivery evidence');
