...
export async function companyInventoryCatalog(tx: InventorySql) {
  ...
  const links: CompanyLink[] = rows.map(row => {
    if (Number(row.key_count) !== 1 || row.target_column !== 'id' || !fields.get(row.table_name)?.has('id')) throw new Error('company_inventory_unsupported_reference');
  ...
  return { links: links.filter(link => related.has(link.target)), fields, tables: [...related].sort(), schema: String(scope.name) };
}
...
export async function companyDeletionInventory(tx: InventorySql, companyId: string, catalog?: Awaited<ReturnType<typeof companyInventoryCatalog>>) {
  ...
  return collectCompanyRows(companyId, source.links, async (link, ids) => {
    ...
    return tx.unsafe(`SELECT id::text, ${source.fields.get(link.table)?.has('company_id') ? 'company_id::text' : 'NULL::text AS company_id'} FROM ${quote(source.schema)}.${quote(link.table)} WHERE ${predicate}`, [JSON.stringify(ids)]);
  });
}
