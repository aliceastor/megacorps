...
test('failed actual Direct Chat result retains supplied billable usage', async t => {
  ...
  t.mock.method(getAdapter('codex-app'), 'dispatch', async () => ({ success: false, output: 'synthetic paid failure', sessionId: 'session-1', tokensUsed: 7, costUsd: 0.00000019, durationSeconds: 1 }));
...
test('failed maintenance retains returned usage', async t => {
  const company: any = { id: randomUUID() };
  ...
  let called = false;
  t.mock.method(getAdapter('hermes-ssh'), 'dispatch', async () => { called = true; return { success: false, output: 'synthetic paid failure', sessionId: 'session', tokensUsed: 4, costUsd: 0.125, durationSeconds: 1 }; });
