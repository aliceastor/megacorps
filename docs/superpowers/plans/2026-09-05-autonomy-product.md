...
Task 2 review identified a concrete human-gate transaction waiting on its own out-of-transaction log insert. Bring the isolated PostgreSQL 16 CI service and explicit test database harness forward into that remediation, with a tests-only RED checkpoint followed by real-database GREEN. Task 9 reuses this harness for accounting concurrency. Task 10 must exercise any remaining deterministic real-database concurrency for merge wait/event closure, approval versus merge initiation, seeded-company retirement versus late references, and budget settlement without duplicating existing coverage. Memory database tests alone do not establish PostgreSQL lock or uniqueness behavior.
