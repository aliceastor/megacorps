...
**落地(2026-09-03)**:migration v18 `merge-closure`(`projects.completion_requires_merge`,gitea-local 新專案預設開)。`merge-gate.ts` 純規則 + 黏合:`selectMergeCandidate` 從 work products 選出 PR 或分支、`planMergeGate` 決定要不要攔(不攔時貼 `merge_gate_skipped` 說明原因:沒設定 / 沒 repo / 沒候選 / 取不到 head)、`parkForMerge` 記下**授權 head SHA** 並把卡停在 `waiting_on_external`、`mergeVerdict` 判 success / drift / failure / ignore。三個核准出口都接了:`reviewCard` 通過、盲審輪 `tryCloseRound` 通過、`PUT /api/approvals/:id` 人類核准。`external-events.ts` 把原本埋在路由裡的狀態轉移抽成 `applyExternalEvent`,Gitea receiver 與逾時掃描共用。receiver 依 `X-Gitea-Event` 分流:`pull_request` closed+merged 且 head 等於授權值 → done;head 不同或 `synchronize` → drift(舊授權作廢、留言 `merge_drift`、卡回 in_review,**panel 卡重開盲審輪**、單審卡重排 review);closed 未合併 → in_progress;`push` 到預設分支則比對 commits,payload 沒列出時才查 Gitea API,且每個 push 事件最多查 5 次(`PUSH_CONTAINMENT_LOOKUPS`)避免放大。`ensureGiteaRepoWebhook` 現在訂閱 `['push','pull_request']`,既有 hook 事件不足會 PATCH 補上,開機 reconcile 會帶著跑。`sweepExternalWaitTimeouts` 進 cron tick。與文件差異:drift 一律走「作廢重審」,沒有做「同一輪自動重新授權」;poll interval 消費者(§13 第 1 項)仍未做。
